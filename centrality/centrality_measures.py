...
def degree_centrality(rel_type, graph):
    ...
    if rel_type == "ATTACKS":
        # Degree centrality
        outdegree_attacks = graph.run(
            "MATCH (u:User) RETURN u.id, size ((u)<-[:" + rel_type + "]-()) AS degree ORDER BY degree DESC",
            rel_type=rel_type).to_data_frame()
        ...
        outdegree_trades = graph.run(
            "MATCH (u:User)RETURN u.id, size ((u)<-[:" + rel_type + "]-()) AS degree ORDER BY degree DESC",
            rel_type=rel_type).to_data_frame()
        indegree_trades = graph.run(
            "MATCH (u:User)RETURN u.id, size (()-[:" + rel_type + "]->(u)) AS degree ORDER BY degree DESC",
            rel_type=rel_type).to_data_frame()
        return outdegree_trades, indegree_trades
    else:
        outdegree_messages = graph.run(
            "MATCH (u:User)RETURN u.id, size ((u)<-[:" + rel_type + "]-()) AS degree ORDER BY degree DESC",
            rel_type=rel_type).to_data_frame()
        indegree_messages = graph.run(
            "MATCH (u:User)RETURN u.id, size (()-[:" + rel_type + "]->(u)) AS degree ORDER BY degree DESC",
            rel_type=rel_type).to_data_frame()
...
def pagerank(rel_type, graph):
    if rel_type == ' ':
        ...
        q3 = graph.run(
            "CALL algo.pageRank.stream('MATCH (u:User) WHERE exists( (u)-[:" + rel_type + "]-() ) RETURN id(u) as id','MATCH (u1:User)-[:" + rel_type + "]-(u2:User) RETURN id(u1) as source, id(u2) as target', {graph:'cypher'}) YIELD nodeId,score with algo.asNode(nodeId) as node, score order by score desc  RETURN node {.id}, score",
            rel_type=rel_type).to_data_frame()
